...
spa.chat = (function () {
  ...
  setPxSizes = function () {
    let px_per_em, window_height_em, opened_height_em;
  ...
  // 引数；
  // * position_typeーenum('closed'、 'opened'、または'hidden')
  // * callbackーアニメーションの最後のオプションのコールバック
  // このコールバックは単一引数としてスライダーdivを表すjqueryコレクションを受け取る。
  // 動作：
  // このメソッドはスライダーを要求された位置に移動する。
  // 戻り値：
  // * trueー要求された位置に移動した
  // * falseー要求された位置に移動していない
  // 例外発行：なし
  //
  ...
  onTapList = function (event) {
    let $tapped = $(event.elem_target), chatee_id;
  ...
  //  そして、要素、イベント、ハンドラを初期化し、ユーザチャットルームインターフェースを提供する。
  ...
  initModule = function ($append_target) {
    ...
    // チャットスライダーHTMLとjQeryキャッシュをロードする
