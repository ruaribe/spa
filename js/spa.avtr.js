...
spa.avtr = (function () {
  ...
  onTapNav = function (event) {
    let css_map,
      $target = $(event.elem_target).closest('.spa-aver-box');
  ...
  onHeldstartNav = function (event) {
    let offset_target_map, offset_nav_map,
  ...
  // 目的：初期化前にモジュール(ユーザセッション中に変更されるはずのない値)を指定する。
  // 動作：
  //  内部データ構造(configMap)を指定の引数で更新する。その他の処理は行わない。
  // 戻り値：なし
  // 例外発行：受け取れない引数または欠如した引数のJavaScriptエラーオブジェクトとスタックトレース
  //
  ...
  // 用例：spa.avatar.initModule( $($container) );
  // 目的：機能を提供するようにモジュールに指示する
  // 引数：$containerー使用するコンテナ
  // 動作：チャットユーザにアバターインターフェースを提供する
  // 戻り値：なし
  // 例外学校：なし
  //
